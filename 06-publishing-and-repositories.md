...
In **Domain Specific Venue**, data can be published in journal data papers, or venues specific to the type of data. For example, in astronomy the data is hosted at the SDSS [@SDSS], Genetic data can be hosted at GenBank [@GenBank], and so on. This is a good option when the data is appropriate for the domain, as it is often required to adhere to community guidelines that facilitate reuse.
