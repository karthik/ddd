...
In contrasst to a CSV data dictionary, JSON-LD has a defined, nested structure, which means more can be communicated efficiently, while still maintaining readability, and avoiding extra writing and repetition that comes with a plain CSV where everything is defined from scratch.

Another rich format is XML, the eXtensible Markup Language. This was an early iteration on the idea of a plain text format that was both human and machine readable. It is very powerful - it powers the entire Microsoft Office suite, but for metadata purposes it is not as human readable. However, one example of its usage is in EML, the Ecological Metadata Language. XML and JSON can be converted into each others respective formats

Expand on:

- The holy grail of the semantic web: JSON-LD
...
If the dataset you are shipping has a research application, the most relevant home for it would be the research data repositories **zenodo** or **dryad**. Zenodo, Launched in 2013 in a joint collaboration between openAIRE and CERN, provides a free, archival location for any researcher to depositor their datasets. The only limits to file sizes are 50gb for individual files, which is generous enough to accommodate a large number of use cases. Zenodo is able to accommodate larger file sizes upon request. Dryad performs data curation
...
tibble::tribble(
  ...
  knitr::kable(caption = "Table Comparing common data submission platforms")
```
