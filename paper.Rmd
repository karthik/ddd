...
There are many features to consider when documenting, sharing, and publishing data. What format to store the data in? How do you describe the variables, observations, and collection methods? Do you include raw data? What about data used in analysis? What about metadata for the variables? And then there's linceses, and long term storage. It can be hard to know where to start. This section discusses challenges that come with these features of data documenting, sharing, and publication, setting up the discussion for the rest of the paper.

## How far to go with data documenting

To frame discussion around the challenges of data documenting, we can think of how an individual dataset fits on two features: "Effort to prepare", and "Ease of understanding", see Figure \@ref(fig:fig-effort-understanding).
...
There are many features to include in data documentation.
Not all of these are needed for every dataset. But, deciding which to include should be based upon the potential impact of the data. The more impactful the data, the more features to include. This is one of the first challanges (tradeoffs) to discuss: how to decide what to include with your data.

Together these all help make the data easier to understand. These features are not clearly described in the scientific literature in the context of including them alongside research.
...
There is also the challenge for format. A simpler format data like CSV (Comma Separated Value) is more reliable than binary serialised or compressed data, but can sometimes be slower to read in and out, and harder to share. 
...
High impact data should be easy to understand and use, but how do you assess its impact?


## Describing the variables and metadata
...
Creating good documentation has the same challenge as doing good writing: it takes time, and it can be hard to know when you are done.
