...
Tooling for producing these this information speeds up the process of sharing data. To help create codebooks, there are R packages such as `dataMeta`, `memisc`. and `codebook`. Codebooks are implemented in other software such as STATA, which provides a "codebook" command. Data can be packaged up in a "data package" with [DataPackageR](https://github.com/RGLab/DataPackageR), which provides tools to wrap up data into an R package, while providing helpers with MD5sum checks that track versioning. Note that is different to [Frictionlessdata's tabular data package spec](https://frictionlessdata.io/docs/tabular-data-package/).
...
Datasets obtained from sensors such as meteorological data are typically easy to prepare and understand.  This is typically because the sensors have to be very specific about what they are measuring, so a lot of the description of the type of data collected happens upstream at the instrument-design level. This flows down to when data is collected. The telescope data from the Sloan Digital Sky Survey (SDSS) is a great example of this. It contains the most detailed, three dimensionsal maps of the universe ever made, and has been operated since 2000, now producing over 200Gb of data every day [@sdss-one-paper; @sdss-four-paper; @sdss-website]. It is not possible to summarise the contents of the data, but briefly, the SDSS data includes photometric and spectroscopic information obtained from a 2.5m telescope at Apache Point Observatory in New Mexico.


The data are are free to use and publicly accessible, with the interface to the data being designed to cover all skills, from shcool children to professional astronomers.
...
The SDSS provides different degrees of understanding. It is a huge project involving many people, and a staggering effort to prepare. Despite this, it is still very easy to understand. This is a further reflection on the idea that high effort  can create highly understandable data, (see Figure \@ref(fig:fig-effort-underrstanding)). The impact of this data is high, having changed the way we understand the past, present and future of the universe itself. This impact is surely due to the care and thought put into making the data accessible and public. It is a statement of what is possible with good data sharing practices.

**include some references to the SDSS project and some of the key/impactful findings**

As an example, the optical and spectral data are explained here: https://www.sdss.org/dr15/spectro/spectro_basics/ and can be downloaded here in special FITS dataformat, or even as a plain CSV: http://skyserver.sdss.org/dr15/en/tools/quicklook/summary.aspx?

An example plot in Figure \@ref(fig:sdss-spectra)A shows the Wavelength against Flux. Here, flux is ..., and wave is ...

```{r prepare-spectra-plot, include = FALSE}
...
```{r sdss-spectra, fig.cap = "Spectra image of wavelength against flux from SDSS. Part A shows the example image taken from SDSS website. Part B shows the replicated figure using data provided"}

plot_grid(plot_spectra, gg_spectra, labels = "AUTO")
```


A great litmus test of reproducibility for this is the fact that from downloading the CSV from the website, a similar data visualisation can be produced, seen in Figure \@ref(fig:sdss-spectra)B: 


The data was downloaded from: http://dr15.sdss.org/optical/spectrum/view/data/format=csv?plateid=1678&mjd=53433&fiberid=425&reduction2d=v5_7_0

This URL contains details of the data, specifically:
...
* fiberid is an integer that marks the fiber number used.
