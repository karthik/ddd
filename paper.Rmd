...
There are many features to include with data alongside publications. However, not all of these are needed for every dataset. This section discusses how to think about the degree of documentation to include. To frame discussion around the challenges of data documenting, we can think of how an individual dataset falls on two features: "Effort to prepare", and "Ease of understanding" in Figure \@ref(fig:fig-effort-understanding).

```{r fig-effort-understanding, fig.cap = "There is a big difference in the effort to prepare data, and how easy it is to understand - look at the difference between most datasets, and something like a Randomized Control Trial (RCT).", out.width = "90%"}

# There are two Axis:
#   X axis: <- Robustness (brittle - robust)
#   Y axis: <- Difficulty (easy - hard)

dat_prep <- tibble(prep = c(15,20,25,90, 96, 98),
               understanding = c(15,20,95,90,96, 80),
                  label = c("Data Dump",
                            "Most studies",
                            "Sensor Data",
                            "50 hectare",
...
To decide the extent of data documentation to include should be based on the impact of the data. The more impactful the data, the more features to include.



To improve data impact it needs more time to prepare so it is easier to understand. Accordingly, data with higher potential for impact and reuse should be made as easy to understand as possible. Therefore data with high potential impact requires more time and effort to prepare. 

High impact data should be easy to understand and use. Impact is hard to measure as a general measure, and may vary from field to field. An example is given below.

For example, data from medical randomized control trials (RCTs) on cancer treatment can have high impact, so requires a lot of time and effort document. Comparatively, a small survey on simple demographics can have low impact. So it might not have a worthwhile tradeoff for ease of understanding and ease of preparation. This means effort put into preparing data documentation for a small survey to be made easy to use should be kept relatively simple and not over complicated. Likewise, data that can be created via simulation from open source software could arguably not be archived - since it can be generated from scratch with code; a reproducible process with likely very low cost in person time to create.

Figure \@ref(fig:data-sharing-workflow) shows the different degrees or steps of this.
...
Creating good documentation has the same challenge as doing good writing: it takes time, and it can be hard to know when you are done. Thinking about the potential impact of should help guide the researcher in how much they should document their data.
...
> A high-level summary, ideally combined with text explaining the structure and nature of the data set, also helps to explain an unfamiliar data set to researchers who want to reproduce analyses or reuse the data.
...
```{r json-code, fig.align = "center", out.width = "75%", fig.cap = "Example snippet of some Table Schema data for a dataset with three variables. This provides a description of each field, and the type of field, and it's description."}
knitr::include_graphics(here::here("figures",
                                   "json-code.png"))
```
...
Tooling for producing these this information speeds up the process of sharing data. Machine-readable metadata that can be indexed by google is created using the `dataspice` package [@dataspice]. To help create codebooks, we recommend the `codebook` package in R [@Arslan2019], which also generates machine readable metadata. Codebooks are implemented in other software such as STATA, which provides a "codebook" command. Data can be packaged up in a "data package" with DataPackageR [@DataPackageR], which provides tools to wrap up data into an R package, while providing helpers with MD5sum checks that track versioning. Note that is different to [Frictionlessdata's tabular data package spec [@frictionlessdata-data-pkg]. 
...
Data that is easy to use should avoid being in a proprietary format that requires special software to read.  For example, a more basic format such as CSV (Comma Separated Value) should be preferred over proprietary data formats, such as SPSS or SAS custom data types, or binary formats, as it is more reliable, although it can sometimes be slower to read in and out, and harder to share due to size.
