...
Another rich format is XML, the e**X**tensible **M**arkup **L**anguage. This was an early iteration on the idea of a plain text format that was both human and machine readable. It is very powerful - it powers the entire Microsoft Office suite, but for metadata purposes it is not as human readable. XML is used is in EML, the Ecological Metadata Language. XML differs to JSON in that JSON is shorter, and quicker and easier to read and write. JSON can also be parsed with standard JavaScript, whereas XML must be parsed by a special XML parser. JSON has key-value pairs of information, whereas XML represents data in a tree structure. XML and JSON can be converted into each others respective formats, for example with the `json` or `xml2` R packages.
...
If the dataset you are shipping has a research application, the most relevant home for it would be the research data repositories **zenodo** or **dryad**. Zenodo, Launched in 2013 in a joint collaboration between openAIRE and CERN, provides a free, archival location for any researcher to depositor their datasets. The only limits to file sizes are 50gb for individual files, which is generous enough to accommodate a large number of use cases. Zenodo is able to accommodate larger file sizes upon request. Dryad will take data from any field of research, and perform human quality control and assistance  of the data, with the ability to link data with a journal publication, but requires a data publishing fee.
