...
Avoid using non-standard licenses, such as those custom written by the authors, as some terms can render them unusable. Once a license is chosen, a LICENSE file should be provided, containing the entire license in the top level of the directory (Figure \@ref(fig:dir-layout)). The license type should be mentioned in the README, if used.
...
Describe how you want your data to be cited in a CITATION file. Citation is important for provenance, and credit. Citing data clarifies the data version used (provenance), and provides authors a citation (credit). To ensure these are provided, data needs to have an accession number, such as a Digital Object Identifier (DOI). A DOI is a permanent, unique identifier for a digital object such as a paper, poster, or software. Access numbers are a prerequisite for citation - if they are unavailable, a citation will be meaningless, as it cannot be tracked by any means. A file named CITATION should be placed in the directory at the top level (Figure \@ref(fig:dir-layout)). It should contain an accession number, and could be in `.bibtex` format. Avenues that provide DOIs are discussed in Section \@ref(avenues).
