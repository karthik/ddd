...
To make data more reusable, we need to make it easier to generate metadata, standardise good practices within and across communities, and make reusable data sharing a recognized, normalised, part of research. Here we present a set of best practices to enable this change. These include documentation (\@ref(readme)), licensing (\@ref(licenses)), citation (\@ref(cite)), code (\@ref(pkgs)), raw and analysis ready data (\@ref(rawdata)). We also discuss considerations for sharing of data of various sizes \@ref(size).
